...
/**
 * @file
 * This provides the Sync Pull from Hubspot to CiviCRM form.
 */

class CRM_Hubspot_Form_Pull extends CRM_Core_Form {

  const QUEUE_NAME = 'mc-pull';
  const END_URL    = 'civicrm/hubspot/pull';
  const END_PARAMS = 'state=done';

  function preProcess() {
    $state = CRM_Utils_Request::retrieve('state', 'String', CRM_Core_DAO::$_nullObject, FALSE, 'tmp', 'GET');
    if ($state == 'done') {
      $stats = CRM_Core_BAO_Setting::getItem(CRM_Hubspot_Form_Setting::HS_SETTING_GROUP, 'pull_stats');
  ...
  public function postProcess() {
    $setting_url = CRM_Utils_System::url('civicrm/hubspot/settings', 'reset=1',  TRUE, NULL, FALSE, TRUE);
    $runner = self::getRunner();
    if ($runner) {
      // Run Everything in the Queue via the Web.
      $runner->runAllViaWeb();
    } else {
      CRM_Core_Session::setStatus(ts('Nothing to pull. Make sure hubspot settings are configured in the <a href='.$setting_url.'>setting page</a>.'));
    }
  }
  ...
  static function getRunner($skipEndUrl = FALSE) {
    ...
    // reset pull stats.
    CRM_Core_BAO_Setting::setItem(Array(), CRM_Hubspot_Form_Setting::HS_SETTING_GROUP, 'pull_stats');
    $stats = array();
    ...
    foreach ($groups as $group_id => $details) {
      ...
      $task  = new CRM_Queue_Task(
        array ('CRM_Hubspot_Form_Pull', 'syncPullList'),
  ...
  static function syncPullList(CRM_Queue_TaskContext $ctx, $listID, $identifier) {
    ...
    $ctx->queue->createItem( new CRM_Queue_Task(
      array('CRM_Hubspot_Form_Pull', 'syncPullCollectHubspot'),
      array($listID),
      "$identifier: Fetching data from Hubspot (can take a mo)"
    ));

    // Add the CiviCRM collect data task to the queue
    $ctx->queue->createItem( new CRM_Queue_Task(
      array('CRM_Hubspot_Form_Pull', 'syncPullCollectCiviCRM'),
      array($listID),
    ...
    $ctx->queue->createItem( new CRM_Queue_Task(
      array('CRM_Hubspot_Form_Pull', 'syncPullUpdates'),
      array($listID),
   ...
   */
  static function syncPullCollectHubspot(CRM_Queue_TaskContext $ctx, $listID) {

    // Shared process.
    $count = CRM_Hubspot_Form_Sync::syncCollectHubspot($listID);
  ...
  static function syncPullCollectCiviCRM(CRM_Queue_TaskContext $ctx, $listID) {
    ...
    // Shared process.
    $stats[$listID]['c_count'] =  CRM_Hubspot_Form_Sync::syncCollectCiviCRM($listID);

    // Remove identicals
    $stats[$listID]['in_sync'] = CRM_Hubspot_Form_Sync::syncIdentical();
  ...
  /**
   * New contacts from Hubspot need bringing into CiviCRM.
   */
  static function syncPullUpdates(CRM_Queue_TaskContext $ctx, $listID) {
    ...
    foreach (CRM_Hubspot_Utils::getGroupsToSync(array(), $listID) as $groupID => $details) {
      // This group is one that we allow Hubspot to update CiviCRM with.
    ...
    CRM_Hubspot_Utils::updateGuessedContactDetails();

    // all Hubspot table
    $dao = CRM_Core_DAO::executeQuery( "SELECT m.*, c.contact_id
      FROM tmp_hubspot_push_m m
      LEFT JOIN tmp_hubspot_push_c c ON m.email = c.email
      ;");
    ...
    while ($dao->fetch()) {
      ...
      $contact_id = CRM_Hubspot_Utils::updateContactDetails($params);
    ...
    // And now, what if a contact is not in the Hubspot list? We must remove them from the membership group.
    ...
    $dao = CRM_Core_DAO::executeQuery( "SELECT c.contact_id
      FROM tmp_hubspot_push_c c
      LEFT OUTER JOIN tmp_hubspot_push_m m ON m.email = c.email
    ...
    CRM_Core_Error::debug_var( 'Hubspot $groupContact= ', $groupContact);
    ...
    CRM_Core_Error::debug_var( 'Hubspot $groupContactRemoves= ', $groupContactRemoves);
    ...
    CRM_Core_DAO::executeQuery("DROP TABLE tmp_hubspot_push_m;");
  ...
  static function updatePullStats($updates) {
    ...
    CRM_Core_BAO_Setting::setItem($stats, CRM_Hubspot_Form_Setting::HS_SETTING_GROUP, 'pull_stats');
  }
